...
// $fba_utilities->better_vardump('',3);// 0- green; 1-red; 2-grey; 3-yellow   
    ...
    function better_vardump($s,$k=3) {
         ...
         $o; // output
            ...
            if(is_array($s)){
                echo "<div style=\"" . $this->i. " padding-left:10px;\">fba_DEBUG :: ".date("Y-m-d h:i:s") . " :: <pre>";
                echo print_r($s);
                echo "</pre></div>";
                ...
                $this->o= "<br /><div style=\"" . $this->i. " padding:25px;\">fba_DEBUG :: ".date("Y-m-d h:i:s") . " :: " . $s . "</div><br />";
                echo  $this->o;
             }//end if
...
// $fetch_type can be
  ...
  $stmt = $this->fba_pdo->prepare($query);
  $stmt->execute($params);
...
// CRSF protection
// ***************************************************************************
// simply insert $fba_utilities->fba_csrf_field() inside a <form></form>
// then check the request ($_POST/$_GET) with fba_csrf_check($request_data)
...
// ***************************************************************************
    ...
    }//end function fba_csrf_field()


    function fba_csrf_check($request_data){

        $fba_csrf_hash_request = (isset($request_data['fba_csrf']))? $request_data['fba_csrf'] : "Request Data CSRF not set" ;
        $fba_csrf_hash_session = (isset($_SESSION['fba_csrf']))? $_SESSION['fba_csrf'] : "Session CSRF not set" ;

        if ($fba_csrf_hash_request==$fba_csrf_hash_session) {
            return true;
        }else{
            header("Location: https://www.fbi.gov/investigate/cyber" );
        }//end if
                 
    }//end function fba_csrf_check($request_data)
