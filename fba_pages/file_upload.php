...
$sql_upload_name_patterns['manual_fba_kw_search'] = "/manual_fba_kw_search/i";
$sql_upload_name_patterns['rocket_fba_kw_search'] = "/AMS Keyword Search/i";
...
if(isset($_POST['submit'])) {
    ...
    $fba_csv_upload_paths = $_FILES['fba_csv_upload']['tmp_name'];
    $fba_csv_upload_names = $_FILES['fba_csv_upload']['name'];

	
	# ----- Looping through the uploaded CSV files ------------------------------------------
	foreach ($fba_csv_upload_names as $fba_csv_filename_key => $fba_csv_filename_value) {
		...
		$upload_file_check = preg_match ($sql_upload_filename_patterns, $fba_csv_filename_value);

		# Report Date
		$upload_date_check = preg_match ($sql_upload_date_pattern, $fba_csv_filename_value,$fba_csv_upload_date);
		if ($upload_date_check == 1){
			$sql_upload_date = str_replace("_"," ",$fba_csv_upload_date[0]);
			$sql_upload_date = date("Y-m-d", strtotime($sql_upload_date));
		}//end if

		# Report Type
		foreach ($sql_upload_name_patterns as $pattern_key => $pattern_value) {
			$upload_type_check = preg_match ($pattern_value, $fba_csv_filename_value);
		...
		if($file_check==3){
			...
			# Get CSV data
			$csv_data = fopen($fba_csv_upload_paths[$fba_csv_filename_key],"r");
			...
			// ---------------------------------------------------------------------------------
				...
				$parfba_uf['kr_campaign_name'] = $sql_campaign_name; 
				$parfba_uf['kr_report_date'] = $sql_upload_date; 
				$file_already_uploaded = $fba_utilities->sql_get_one_row($query_uf, $parfba_uf);
				if($file_already_uploaded) $upload_dupl_check=0; else $upload_dupl_check = 1; 

				$fba_utilities->better_vardump($csv_data_all_lines,1); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
				...
				if($upload_dupl_check==1 && $campaign_name_check == 1){
					...
					foreach ($csv_data_all_lines as $csv_key => $csv_value) {
						...
						$parfba_kr['kr_report_date'] = $sql_upload_date; 
						$parfba_kr['kr_campaign_name'] = $sql_campaign_name; 

						foreach ($kr_datafields as $krd_key => $krd_value) {
							$parfba_kr[$krd_value] = $csv_data_all_lines[$csv_key][$krd_key];
						}//end foreach
					
$meta_parfba_kr[] = $parfba_kr;//// TEST // TEST // TEST // TEST // TEST // TEST // TEST // TEST // TEST // TEST // TEST 
						// -------- 3 - PDO Execution ---------------------------------------------------------
										
						// $fba_utilities->sql_execute($query_kr, $parfba_kr);

					}//end foreach ($csv_data_all_lines as $csv_key => $csv_value)


				$fba_utilities->better_vardump($csv_data_all_lines,2); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
				// $fba_utilities->better_vardump($meta_parfba_kr,3); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
			...
			// ---------------------------------------------------------------------------------
				...
				// $fba_utilities->better_vardump("ASIN Campaign cannot be processed As of 2020-04-10",3);  //TEST  //TEST  //TEST  //TEST  //TEST 
			...
			// ---------------------------------------------------------------------------------
				...
				$parfba_uf['cr_report_date'] = $sql_upload_date; 
				$file_already_uploaded = $fba_utilities->sql_get_one_row($query_uf, $parfba_uf);
				if($file_already_uploaded) $upload_dupl_check=0; else $upload_dupl_check = 1; // REVISE WHAT TO DO WITH THAT INFORMATION

// $fba_utilities->better_vardump($upload_dupl_check,3); exit();//VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
				if($upload_dupl_check==1){
					...
					foreach ($csv_data_all_lines as $csv_key => $csv_value) {
						...
						$parfba_cr['cr_report_date'] = $sql_upload_date; 

						foreach ($cr_datafields as $key => $value) {
							$parfba_cr[$value] = $csv_data_all_lines[$csv_key][$key];
						}//end foreach
					
						// -------- 3 - PDO Execution ---------------------------------------------------------
						$fba_utilities->sql_execute($query_cr, $parfba_cr);
					}//end foreach ($csv_data_all_lines as $csv_key => $csv_value)
			...
			// ---------------------------------------------------------------------------------
			 	case 'manual_fba_kw_search':
			 	case 'rocket_fba_kw_search':
				...
				foreach ($csv_data_all_lines as $csv_key => $csv_value) {
					...
					$parfba_kh['kh_keyword_text'] = $csv_data_all_lines[$csv_key][0]; 
					$parfba_kh['kh_keyword_author'] = $csv_data_all_lines[$csv_key][1]; 

				// $fba_utilities->better_vardump($query_kh,2); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
				// $fba_utilities->better_vardump($parfba_kh,2); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
				// exit();// EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV // EXIT DEV 


					// -------- 3 - PDO Execution ---------------------------------------------------------
					$fba_utilities->sql_execute($query_kh, $parfba_kh);
				...
				// $fba_utilities->better_vardump("<br>File type UNKNOWN: $fba_csv_filename_key => $fba_csv_filename_value",3); 
			...
			if($file_success){
				...
				$parfba_ufi['uf_filename'] = $fba_csv_filename_value; 
				$parfba_ufi['uf_upload_date'] = $sql_upload_date; 
						
				$fba_utilities->sql_execute($query_ufi, $parfba_ufi); 
		...
		$uf_report[$uf_report_id]['filename'] = $fba_csv_filename_value;
	...
	}//end foreach ($fba_csv_upload_names as $fba_csv_filename_key => $fba_csv_filename_value) 
...
// $fba_utilities->better_vardump($acr_display_rows,3); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
// exit();		
	...
	foreach ($uf_report as $ufr_key => $ufr_upload) {
		...
		if($ufru_key=='camp'&& $sql_upload_type!=="keywords")continue;
  ...
  include "fba_includes/upload_form.php";

}//end if(isset($_POST['submit'])) 


// $fba_utilities->better_vardump($file_already_uploaded,2); //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV //VARDUMP DEV 
// exit();
