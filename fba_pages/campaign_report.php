...
// $fba_utilities->better_vardump($q_string,1);// 0- green; 1-red; 2-grey; 3-yellow   
// $fba_utilities->better_vardump($q_array,2);// 0- green; 1-red; 2-grey; 3-yellow   
// exit();
...
// Retrieve active campaigns - List (active as of last report date on file)
...
$parfba_active_campaigns = []; 

$cr_active_campaigns = $fba_utilities->sql_get_all_rows($query_active_campaigns, $parfba_active_campaigns);


// $fba_utilities->better_vardump($cr_active_campaigns,2);// 0- green; 1-red; 2-grey; 3-yellow   
// $fba_utilities->better_vardump("<br>cr_active_campaigns_report Done ",0);// 0- green; 1-red; 2-grey; 3-yellow   
// exit();
...
foreach ($cr_active_campaigns as $cac_key => $cac_value) {
	...
	$parfba_active_campaigns['cr_campaign_name'] = $cac_value['cr_campaign_name'];

	$cr_active_campaigns_report[] = $fba_utilities->sql_get_all_rows($query_active_campaigns, $parfba_active_campaigns);

}//end foreach ($kw_harvest_raw as $acr_key => $acr_value)



// $fba_utilities->better_vardump($query_active_campaigns,3);// 0- green; 1-red; 2-grey; 3-yellow   
// $fba_utilities->better_vardump($parfba_active_campaigns,3);// 0- green; 1-red; 2-grey; 3-yellow   
// $fba_utilities->better_vardump($cr_active_campaigns_report,2);// 0- green; 1-red; 2-grey; 3-yellow   
// exit();



//Tally data to last report day
$sum_imp_all = 0; 
$sum_clk_all = 0; 
$sum_ord_all = 0; 	

//Tally data to prevoous to last report day
$sum_imp_all_1 = 0; 
$sum_clk_all_1 = 0; 
$sum_ord_all_1 = 0; 	
...
foreach ($cr_active_campaigns_report as $acr_campaign_key => $acr_campaigns) {
	...
	foreach ($acr_campaigns as $acr_campaign_row_key => $acr_campaign_row_value) {
			...
			if($acr_campaign_row_key==0){
				$sum_imp_all += $this_day_imp;
				$sum_clk_all += $this_day_clk;
				$sum_ord_all += $this_day_ord;
			}//end if
		...
		if($acr_campaign_row_key < $acr_last_ley){
			...
			if($acr_campaign_row_key==0){
				$sum_imp_all_1 += $previous_day_imp;
				$sum_clk_all_1 += $previous_day_clk;
				$sum_ord_all_1 += $previous_day_ord;
			}//end if
...
echo "<br>$sum_ord_all"; 	
//Tally data to prevoous to last report day
echo "<br>$sum_imp_all_1"; 
echo "<br>$sum_clk_all_1"; 
echo "<br>$sum_ord_all_1"; 	
